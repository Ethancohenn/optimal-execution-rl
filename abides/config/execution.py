...
def _rand_seed():
    # numpy on some Windows/Py3.6 builds rejects randint(high=2**32) for int32.
    return int(np.random.randint(low=0, high=(2 ** 31 - 1)))
...
symbols = {symbol: {'r_bar': r_bar,
                    ...
                    'megashock_mean': 1e3,
                    'megashock_var': 5e4,
                    'random_state': np.random.RandomState(seed=_rand_seed())}}
...
kernel = Kernel("Kernel", random_state=np.random.RandomState(seed=_rand_seed()))
...
# LATENCY
latency_rstate = np.random.RandomState(seed=_rand_seed())
