...
def _episode_is_from_traj(traj_df: pd.DataFrame) -> pd.Series:
    ...
    if not required.issubset(set(traj_df.columns)):
        ...
        raise ValueError(f"Trajectories missing columns for IS computation: {missing}")

    step_is = traj_df["executed_qty"] * (traj_df["mid_price"] - traj_df["exec_price"])
    return step_is.groupby(traj_df["episode"]).sum().astype(float)


def _episode_reward_objective_from_traj(traj_df: pd.DataFrame) -> pd.Series:
    # Base aligned reward term: -IS
    reward = -_episode_is_from_traj(traj_df)
    if "hold_penalty" in traj_df.columns:
        hold = traj_df.groupby("episode")["hold_penalty"].sum().astype(float)
        reward = reward - hold
    if "leftover_penalty" in traj_df.columns:
        left = traj_df.groupby("episode")["leftover_penalty"].sum().astype(float)
        reward = reward - left
    return reward
...
def main() -> None:
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Override episode_reward with objective reward computed from trajectories "
            "for both runs: -episode_is - leftover_penalty_sum(if present)."
        ),
    )
    args = p.parse_args()
    ...
    if args.reward_from_step_is:
        rl_reward = _episode_reward_objective_from_traj(rl_traj_eval).values
        baseline_reward = _episode_reward_objective_from_traj(baseline_traj).values
    else:
    ...
    fig.suptitle(
        (
            f"{args.rl_label} vs {args.baseline_label} - Strategy Comparison "
            f"({args.rl_label} eval: last {args.eval_last} episodes)"
        ),
        fontsize=13,
        fontweight="bold",
        y=1.01,
    ...
    ax4 = axes[3]
    bars = ax4.bar(
        [args.rl_label, args.baseline_label],
    ...
    winner_label = args.rl_label if rl_comp >= baseline_comp else args.baseline_label
    winner_x = 0.25 if winner_label == args.rl_label else 0.75
    ...
    print("\nSummary")
    print(f"{'Metric':<30} {args.rl_label:>12} {args.baseline_label:>12} {'Winner':>12}")
    print("-" * 72)
    rows = [
    ...
    for name, rv, bv, prefer in rows:
        if prefer == "lower":
            winner = args.rl_label if rv < bv else args.baseline_label
        elif prefer == "higher":
            winner = args.rl_label if rv > bv else args.baseline_label
        else:
            winner = args.rl_label if abs(rv) < abs(bv) else args.baseline_label
