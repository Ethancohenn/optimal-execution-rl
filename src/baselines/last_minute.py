...
# Allow running as either `python -m src.baselines.last_minute` or `python src/baselines/last_minute.py`.
...
def build_run_dir(base_dir: str, run_name: str | None, overwrite: bool) -> str:
    if run_name is None:
        run_name = datetime.now().strftime("last_minute_%Y%m%d_%H%M%S")
...
def run_last_minute(args: argparse.Namespace) -> str:
    if args.use_abides:
        env = AbidesReplayEnv(
        ...
        print(f"[last_minute] Using AbidesReplayEnv  npz={args.npz_path} (split={args.split})")
    else:
        # Allow full liquidation in one step at the deadline on synthetic env.
        env = StubExecutionEnv(T=args.T, Q0=args.Q0, max_trade_size=args.Q0, seed=args.seed)
        print("[last_minute] Using synthetic StubExecutionEnv")
    ...
    for episode in range(args.episodes):
        ...
        while not done:
            target_qty = remaining if step_idx == (int(args.T) - 1) else 0.0
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a last-minute-liquidation baseline on an execution env.")
...
if __name__ == "__main__":
    run_path = run_last_minute(parse_args())
    print(f"Saved last-minute baseline artifacts to: {run_path}")
