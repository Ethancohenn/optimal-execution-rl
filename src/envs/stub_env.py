...
class StubExecutionEnv(gym.Env):
    ...
    def __init__(self, T: int = 20, Q0: float = 1000.0, max_trade_size: float | None = None, seed: int = 0):
        super().__init__()
        self.T = int(T)
        self.Q0 = float(Q0)
    ...
    def step(self, action: int):
        ...
        executed_qty = action_to_qty(action, self.remaining_inventory, self.max_trade_size)

        # impact: worse price if you trade more (simple)
        impact = 0.0002 * executed_qty
        exec_price = self.mid_price - impact  # selling: lower is worse

        # reward: negative implementation shortfall vs mid
        # selling: IS = sum(q*(mid - exec_price)) >= 0 ; reward = -IS step
        step_is = executed_qty * (self.mid_price - exec_price)
        reward = -float(step_is)

        self.remaining_inventory = float(self.remaining_inventory - executed_qty)

        self.t += 1
        terminated = (self.t >= self.T)
        truncated = False

        # penalty if not finished at end
        if terminated and self.remaining_inventory > 1e-6:
            reward -= float(0.01 * self.remaining_inventory)

        obs = self._get_obs()
        info = self._get_info(executed_qty=executed_qty, exec_price=exec_price)
    ...
    def _get_info(self, executed_qty: float, exec_price: float):
        return {
